...
interface SortLabelMap {
  [key: string]: string;
}

const VideosTab: React.FC<VideosTabProps> = (props) => {
  const [videos, setVideos] = useState<Video[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState<boolean>(true);
  const [contentState, setContentState] = useContext(contentStateContext);
  const fetchedPagesRef = useRef<Set<string>>(new Set()); // key = `${sortBy}-${page}`
  const videoCacheRef = useRef<{ [key: string]: Video[] }>({}); // key: `${sortBy}-${page}` → video[]
  const VIDEO_CACHE_STORAGE_KEY = "cachedVideosBySort";

  const pageRef = useRef<number>(0); // Track page here without triggering re-renders
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(VIDEO_CACHE_STORAGE_KEY, (result) => {
      if (result?.[VIDEO_CACHE_STORAGE_KEY]) {
        videoCacheRef.current = result[VIDEO_CACHE_STORAGE_KEY] as { [key: string]: Video[] };
  ...
  const handleCopyLink = (videoId: string): void => {
    if (!CLOUD_FEATURES_ENABLED) {
      // show only local placeholder videos
      return;
    }
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(["sortBy"], (result) => {
      if (result.sortBy && !contentState.sortBy) {
        setContentState((prev: any) => ({ ...prev, sortBy: result.sortBy }));
      }
    });
  }, []);

  const sortLabelMap: SortLabelMap = {
            ...
            >
              ...
              >
                <DropdownMenu.RadioGroup
                  value={sortBy}
                  onValueChange={(value) => {
                    setContentState((prev: any) => ({ ...prev, sortBy: value }));
