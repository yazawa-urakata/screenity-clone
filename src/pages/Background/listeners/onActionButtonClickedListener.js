...
const handleTabMessaging = async (tab) => {
  ...
  try {
    ...
    if (targetTab) {
      sendMessageTab(activeTab, { type: "stop-recording-tab" }).catch((error) => {
        console.log("Could not send stop-recording message to active tab:", error);
      });
    } else {
      sendMessageTab(tab.id, { type: "stop-recording-tab" }).catch((error) => {
...
const openPlaygroundOrPopup = async (tab) => {
  ...
  if ((!isForbidden || isPlaygroundOrSetup) && navigator.onLine) {
    sendMessageTab(tab.id, { type: "toggle-popup" }).catch((error) => {
    ...
    const onUpdated = (tabId, changeInfo, updatedTab) => {
      if (updatedTab.id === newTab.id && changeInfo.status === "complete") {
        ...
        setTimeout(() => {
          sendMessageTab(newTab.id, { type: "toggle-popup" }).catch((error) => {
  ...
  chrome.action.onClicked.addListener(async (tab) => {
    try {
      ...
      if (firstTime && tab.url.includes(chrome.runtime.getURL("setup.html"))) {
        ...
        sendMessageTab(activeTab.id, { type: "setup-complete" }).catch((error) => {
