...
export const setupHandlers = (): void => {
  ...
  registerMessage("editor-ready", async (message) => {
    ...
    if (msg.newProject) {
      ...
      const res = await fetch(
        `${API_BASE}/videos/${msg.projectId as string}/auto-publish`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    if (msg.publicUrl) {
      copyToClipboard(msg.publicUrl as string);
    }

    if (messageTab) {
      sendMessageTab(messageTab, {
        type: "update-project-ready",
        share: Boolean(msg.publicUrl),
        newProject: Boolean(msg.newProject),
        sceneId: sceneId,
      });
    } else {
  ...
  registerMessage("finish-multi-recording", async () => {
    ...
    try {
      ...
      if (!recordingToScene) {
        ...
        const res = await fetch(
          `${API_BASE}/videos/${multiProjectId as string}/auto-publish`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const publicUrl = `${process.env.SCREENITY_APP_BASE}/view/${multiProjectId as string}/`;
        ...
        if (editorTab) {
          focusTab(editorTab as number);
          sendMessageTab(editorTab as number, {
            type: "update-project-ready",
            share: false,
            newProject: false,
          });
