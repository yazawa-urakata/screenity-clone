...
export const handleChunks = async (chunks: any[], override = false) => {
  ...
  if (bannerSupport) {
    sendMessageTab(sandboxTab as number, { type: "banner-support" });
  }

  const sendBatch = async (batch: any[], retryCount = 0) => {
    try {
      const response = await sendMessageTab(sandboxTab as number, {
        type: "new-chunk-tab",
        chunks: batch,
      });
